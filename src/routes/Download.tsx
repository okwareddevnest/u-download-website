  ...
  return (
    <div className="container-app py-12">
      ...
      {!state.loading && !state.error && (
        <div>
          {state.selected ? (
            <div className="rounded-xl border border-slate-800 p-6 shadow-sm">
              <div className="mb-2 text-sm uppercase tracking-wide text-slate-400">
                Recommended for {state.platform}
              </div>
              <div className="mb-4 text-xl font-semibold text-slate-100">{state.selected.filename}</div>
          ...
          <div className="mt-10">
            ...
            <div className="grid gap-3">
              {state.assets.map((a) => (
                <div key={a.url} className="flex items-center justify-between rounded-lg border border-slate-700 p-3 text-sm">
